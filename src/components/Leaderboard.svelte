        ...
        <div class="tabs-scroll">
            <div class="tabs">
                <button
                    ...
                    on:click={() => switchTab("stars")}
                >
                    🌟 星星
                </button>
                <button
                    class:active={activeTab === "hard_mode"}
                    on:click={() => switchTab("hard_mode")}
                >
                    🔥 困難
                </button>
                <button
                    class:active={activeTab === "combo"}
                    on:click={() => switchTab("combo")}
                >
                    ☄️ 連擊
                </button>
                <button
                    class:active={activeTab === "fastest"}
                    on:click={() => switchTab("fastest")}
                >
                    ⚡️ 競速
                </button>
                <button
                    class:active={activeTab === "hardest"}
                    on:click={() => switchTab("hardest")}
                >
                    💀 魔王
                </button>
            </div>
                ...
                <div class="hardest-list">
                    {#each hardestQuestions as q, i}
                        <div class="hardest-item">
                            ...
                            <span class="wrong-rate"
                                >{calculateWrongRate(q)}% 答錯</span
                            >
                        </div>
                    {:else}
                        <div class="empty">
                            還沒有足夠的統計資料 🧐<br /><small
                                >需要至少 10 次答題才會顯示</small
                            >
                        </div>
                    {/each}
                </div>
            {:else}
                ...
                <div class="ranking-list">
                    {#each leaderboard as entry, i}
                        {@const isMe =
                            entry.players?.browser_id === currentBrowserId}
                    ...
                    {:else}
                        <div class="empty">
                            還沒有人上榜 🏃‍♂️<br /><small>成為第一個吧！</small>
                        </div>
                    {/each}
                </div>
            {/if}
