<script>
  ...
  async function handleAnswer(option) {
    ...
    const correctPinyin =
      currentQuestion.options.find((o) => o.isCorrect)?.pinyin || "";
    recordQuestionAttempt(
      questionId,
    ...
    if (isHardMode) {
      ...
      if (score > hardModeHighScore) {
        ...
        localStorage.setItem(
          "zhuyin_hard_high_score",
          hardModeHighScore.toString(),
        );
      }
      ...
      if (playerId) {
        const maxLevel = parseInt(
          localStorage.getItem("zhuyin_max_level") || "1",
        );
  ...
  onMount(async () => {
    ...
    const savedStars = parseInt(
      localStorage.getItem("zhuyin_total_stars") || "0",
    );
    // Hard Mode
    const savedHardUnlock =
      localStorage.getItem("zhuyin_hard_mode_unlocked") === "true";
    isHardModeUnlocked = savedHardUnlock;
    hardModeHighScore = parseInt(
      localStorage.getItem("zhuyin_hard_high_score") || "0",
    );
    const savedCombo = parseInt(
      localStorage.getItem("zhuyin_combo_count") || "0",
    );
        ...
        <div class="result-card milestone" in:scale>
          <div class="confetti-container">
            {#each confettiParticles as p}
              <div
                class="confetti"
                style="left: {p.left}; animation-delay: {p.animationDelay}; background: {p.backgroundColor}"
              ></div>
            {/each}
          </div>
          <h1>🏆 里程碑達成！</h1>
          <p class="milestone-text">
            恭喜 <strong>{playerName}</strong><br />通過第 {currentLevel} 關！<br
            />你是注音小天才！
          </p>
