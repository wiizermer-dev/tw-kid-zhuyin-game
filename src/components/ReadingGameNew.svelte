<script>
  ...
  import { playCorrectSound, playWrongSound, playComboSound, playLevelUpSound, speakWord } from "../lib/audio.js";
  ...
  function finishLevel() {
    ...
    if (!completedLevels.includes(level)) {
      ...
      localStorage.setItem('zhuyin_completed_levels', JSON.stringify(completedLevels));
    }
    
    // 保存星級
    const starsData = JSON.parse(localStorage.getItem('zhuyin_stars_per_level') || '{}');
    ...
    // 更新總星數
    const totalStars = parseInt(localStorage.getItem('zhuyin_total_stars') || '0');
  ...
  <div class="game-header">
    ...
    {#if chapterInfo}
      <div class="chapter-badge">
        {chapterInfo.icon} {chapterInfo.name}
      </div>
    {/if}
    ...
    <div class="game-content">
      ...
      <div class="progress-section">
        <div class="progress-bar">
          <div 
            class="progress-fill" 
            style="width: {((currentIndex + 1) / questions.length) * 100}%; background: {chapterInfo?.color || '#4A90E2'};"
          ></div>
        </div>
      ...
      {#if questions.length > 0}
        ...
        {#key currentQuestion.id}
          ...
          >
            ...
            <div class="word-display">
              ...
              <p class="english-word">{currentQuestion.english}</p>
            </div>
        ...
        {#if showFeedback}
          <div class="feedback-overlay" transition:fade={{ duration: 200 }}>
            <div class="feedback-icon" in:scale={{ duration: 300, easing: elasticOut }}>
              {#if isCorrect}
    ...
    <div class="complete-screen" in:scale>
      <div class="complete-card postcard">
        ...
        <div class="action-buttons">
          ...
          <button class="btn-primary" on:click={nextLevel}>
            ▶️ 下一關
          </button>
        </div>
        
        <button class="btn-outline" on:click={onBack}>
          🗺️ 返回地圖
        </button>
      </div>
    </div>
